...
def main(task):
    ...
    for i, question in enumerate(questions):
        if question['ts_name'] not in results.keys():
            if evaluated_model == "claude-3-haiku" or evaluated_model == "bedrock/us.anthropic.claude-3-7-sonnet-20250219-v1:0":
...
if __name__ == "__main__":
    tasks = ['ts_retrieval_perturbed', "caption_retrieval_perturbed", 
             "ts_comparison/amplitude",
             "ts_comparison/peak_earlier",
             "ts_comparison/volatility",
             "ts_comparison/mean"]
    for task in tasks:
        main(task)