...
def retry_on_rate_limit(func, max_retries=5, initial_delay=2):
...
def get_response(prompt,
    ...
    def process_prompt(p):
        if model == "OpenAI GPT-4o" or model == "GPT-4o" or model == "OpenAI GPT-5" or model == "OpenAI GPT-5":
            # Read OpenAI API key
            ...
            # Set AWS credentials
            with open("/home/ubuntu/thesis/.credentials/aws", "r") as cred_file:
                A, B, C = [line.strip() for line in cred_file.readlines()]

            os.environ["AWS_ACCESS_KEY_ID"] = A
            os.environ["AWS_SECRET_ACCESS_KEY"] = B
            os.environ["AWS_REGION_NAME"] = C
        ...
        elif model == "Google Gemini-2.0-Flash" or model == "gemini-2.0-flash":
          # API key approach:
          model_name = "gemini-2.0-flash"
          with open("/home/ubuntu/thesis/.credentials/google pratham", "r") as file:
          ...
          response = retry_on_rate_limit(make_request)
          text_response = response.text
          return text_response

          # Vertex AI approach (commented out):
          # client = genai.Client(
          #   vertexai=True,
        ...
        elif model == "Google Gemini-3-Pro-Preview" or model == "gemini-3-pro-preview":
          # API key approach
          with open("/home/ubuntu/thesis/.credentials/google pratham", "r") as file:
              google_api_key = file.read().strip()
          client = genai.Client(api_key=google_api_key)
        ...
        elif model == "OpenAI GPT-5.1" or model == "gpt-5.1":
          with open("/home/ubuntu/thesis/.credentials/openai", "r") as file:
              openai_api_key = file.read().strip()
        ...
        elif model == "OpenAI GPT-5-mini" or model == "gpt-5-mini":
          with open("/home/ubuntu/thesis/.credentials/openai", "r") as file:
              openai_api_key = file.read().strip()
...
def oracle_score(generated_caption, gt_caption, model="Google Gemini-2.0-Flash"):
  ...
  The scoring criteria is based on Semantic Similarity: How closely does the generated caption convey the same meaning as the ground truth? When Then the generated caption tells different information, that is not an error. There's an error when the generated captions says something contraddictory to the ground truth.The order of the facts and linguistic style do not matter.
  
  Examples:
  
  Generated: "Pleateau in the beginning\nStrong fluctuations.\nMinimum point in the end"
  Ground Truth: "Steady increase in the beginning\nMinimum value in the end\nNegligible Fluctuations throughout"
  Score:
  - Overall: 30
  
  Generated: "Decrease in the beginning\n.Peaks near the middle\nDeclining from the last 2/3"
  Ground Truth: "Steady decrease initially\nLow in the middle\nDrop in the last portion"
  Score:
  - Overall: 65

  Generated: "Initially rising\nDips by the end\nPeaks twice"
  Ground Truth: "Initially increasing\nDrops in the end\nHas two peaks\n"
  Score:
  - Overall: 100
...
def generate_prompt_for_baseline(dataset_name, metadata, ts):
  ...
  if dataset_name == "air quality":
    request = f"""Here is a time series about {metadata["sampling frequency"]} {metadata["measure"]} in the Indian city of {metadata['city']}. 
    The starting time is {metadata["starting time"]} for the series. \n {ts} \n
    \nDescribe this time series following these rules:
    
    1. specify and include all contextual data that is needed.
    2. make statistical comparisons (max, min, mean, standard deviation) by providing numerical values.
    3. comment on patterns, shapes, and trends of the time series.
    4. explain how the period compares to historical or global norms.
    5. describe the nature of the movements, patterns, and anomalies if any.
    6. walk through the timeline and describe sub-periods, phases, and periodic patterns when present.
    7. Report dates when events happened.
      
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
          f
    Here is some historical context:
     - all-time maximum: {metadata.get("all-time  maximum", "NULL")}
     - all-time minimum: {metadata.get("all-time minimum", "NULL")}
     - all-time average value until today: {metadata.get("all-time average value until today", "NULL")}
     - all-time standard deviation until today: {metadata.get("all-time standard deviation until today", "NULL")}
    
    Use descriptive language to create engaging, natural-sounding text.
    Avoid repetitive phrasing and overused expressions.

    Answer in a single paragraph of four to five sentences at most, without bullet points or any formatting.
    """

  elif dataset_name == "crime":
    request = f"""Here is a time series about the number of {metadata["sampling frequency"]} crimes committed in {metadata["town"]}, Los Angeles, from {metadata["start date of the series"]} to {metadata["end date of the series"]}: \n {ts}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here is some historical context:
     - historical maximum in the history of this town: {metadata.get("general maximum in the history of this town", "NULL")}
     - historical minimum in the history of this town: {metadata.get("general minimum in the history of this town", "NULL")}
     - historical mean in the history of this town: {metadata.get("general mean in the history of this town", "NULL")}
     - historical standard deviation in the history of this town: {metadata.get("general standard deviation in the history of this town", "NULL")}
    ...
    request = f"""Here is a time series about the number of {metadata['sampling frequency']} {metadata['means']} crossing the port of {metadata['port']} at the {metadata["border"]} border, from {metadata["start date of the series"]} to {metadata["end date of the series"]}:\n{ts}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here is some historical context:
     - all-time maximum of this port: {metadata.get("general maximum in the history of this port", "NULL")}
     - all-time minimum of this port: {metadata.get("general minimum in the history of this port", "NULL")}
     - all-time mean of this port: {metadata.get("general mean in the history of this port", "NULL")}
     - all-time standard deviation of this port: {metadata.get("general standard deviation in the history of this port", "NULL")}
    ...
    request = f"""I will give you a time series about the {metadata['sampling frequency']} {metadata['attribute']} of {metadata['country']} from {metadata['starting year']} to {metadata['end year']}. It is measured as number per 1000 people. {metadata['country']} is categorized as a country with these attributes: {metadata['category by income']}.
    Here is the time series: \n {ts}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here is some global context:
     - average time series globally: {metadata.get("global average time series", "NULL")}
     - standard deviation globally: {metadata.get("global standard deviation", "NULL")}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here is some broader context:
     - average time series of this type of location ({metadata['geotype']}): {metadata.get("average time series of this type of location", "NULL")}
     - standard deviation of this type of location ({metadata['geotype']}): {metadata.get("standard deviation of this type of location", "NULL")}
    ...
    request = f"""I will give you a time series about the {metadata['sampling frequency']} {metadata['attribute']} due to COVID-19 in the country of {metadata['country']}, {metadata['region']}. The country has a population of {metadata['population']} and is classified as {metadata['income group']}. {f"The country has a GDP per capita of {metadata['gdp per capita']}." if 'gdp per capita' in metadata.keys() else ""}
    {f"The percentage of over 65 is {metadata['over 65']}." if 'over 65' in metadata.keys() else ""} {f"The median age in the country is {metadata['median age']}." if 'median age' in metadata.keys() else ""} {f"The country has a population density of {metadata['population density']}." if 'population density' in metadata.keys() else ""}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here is some historical context:
     - all-time maximum in this country: {metadata.get("historical maximum in this country", "NULL")}
     - all-time minimum in this country: {metadata.get("historical minimum in this country", "NULL")}
     - all-time mean in this country: {metadata.get("historical mean in this country", "NULL")}  
     - all-time standard deviation in this country: {metadata.get("historical standard deviation in this country", "NULL")}
   ...
   \nDescribe this time series following these rules:
    ...
    2. make statistical comparisons (max, min, mean, standard deviation) by providing numerical values.
    3. comment on patterns, shapes, and trends of the time series.
    4. describe the nature of the movements, patterns, and anomalies if any.
    5. walk through the timeline and describe sub-periods, phases, and periodic patterns when present.
    6. Report dates when events happened.
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here is some historical context:
     - historical maximum in this country: {metadata.get("historical maximum in this country", "NULL")}
     - historical minimum in this country: {metadata.get("historical minimum in this country", "NULL")}  
     - historical mean in this country: {metadata.get("historical mean in this country", "NULL")}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here's some additional historical context:
     - historical best week: {metadata.get("best week", "NULL")}, with sales {metadata.get("best week sales", "NULL")}
     - historical worst week: {metadata.get("worst week", "NULL")}, with sales {metadata.get("worst week sales", "NULL")}
    ...
    **Not all information for the above rules might be present or needed**, focus on information that is actually evident in the data inputs.
    ...
    Here's some additional historical context:
     - historical maximum: {metadata.get("historical max", "NULL")}
     - historical minimum: {metadata.get("historical min", "NULL")}
     - historical mean: {metadata.get("historical mean", "NULL")}
...
def extract_num_dict_from_text(caption, model="Google Gemini-2.0-Flash"):
  prompt = f"""
    I will provide you with a paragraph of text, which described a time series. Your job is to extract important numbers from it. Specifically, you have to extract minimum, maximum, mean, and standard deviation of this specific time series (not historical statistics) from the text if the text mentions them. You must ignore any statistic about the general history or all-time. For example, you should ignore statistics like "historical maximum", "all-time mean" or something like that. I only need you to extract statistics of the current specific time series.
   ...
   IMPORTANT: For each statistic (minimum, maximum, mean, std):
   - If the text DOES NOT mention or attempt to report that statistic at all, set its value to null
   - If the text DOES mention or attempt to report that statistic (even if the value is uncertain or approximate), provide the numerical value

   The key distinction is whether the caption attempts to report the statistic or not. If it attempts to report it, extract the number. If it doesn't attempt to report it at all, use null.
...
def extract_num_dict_from_dict(metadata, model="Google Gemini-2.0-Flash"):
  prompt = f"""
    I will provide you with a json dictionary consisting of some metadata of a time series. Your job is to extract some numbers from it. Specifically, you have to extract the minimum, maximum, mean, and standard deviation of this specific time series. Sometimes they are absent from the data and in these cases you should report a null value. You must ignore historical or all-time statistics as they are not the statistics that I need for this current specific time series. 
    
    For example, you should ignore statistics like "historical max", "historical standard deviation", "all-time max", and "all-time mean" or anything like that. Focus instead on the specific statistics of the current time series.
...
def create_bottom_earlier_question(ts_path1, ts2, prompt_save_folder, answer_save_folder):
  ...
  min_idx_ts1 = ts1.index(min(ts1))
  min_idx_ts2 = ts2.index(min(ts2))
...
def main():
  ...
  print(get_response("How are you?", model="gemini-3-pro-preview"))
  
  #print(get_response("How are you?", model="Google Gemini-2.0-Flash"))
  #print(get_response("How are you?", model="OpenAI GPT-4o"))
  
  
  """# Load the JSON file
  json_path = "/home/ubuntu/thesis/data/CaTSBench/hard_questions_small/ts_retrieval_perturbed/tasks.json"
  csv_path = "/home/ubuntu/thesis/data/CaTSBench/hard_questions_small/ts_retrieval_perturbed/tasks.csv"
  ...
  #print(get_response("how are you today?", model="gemini-2.5-pro"))
  
  """for filename in os.listdir("/home/ubuntu/thesis/data/evaluation results"):
    if filename.endswith(".json"):
      file_path = os.path.join("/home/ubuntu/thesis/data/evaluation results", filename)
      with open(file_path, "r") as file:
        data = json.load(file)

      numeric_score_2_fields = ["maximum", "mean", "minimum", "std"]
      totals = {field: 0.0 for field in numeric_score_2_fields}
      counts = {field: 0 for field in numeric_score_2_fields}

      for category, scores in data.items():
        if category == "average":
          continue
        numeric_2 = scores.get("numeric score 2.0", {})
        for field in numeric_score_2_fields:
          value = numeric_2.get(field)
          if value is not None:
            totals[field] += value
            counts[field] += 1

      averages = {field: (totals[field] / counts[field]) if counts[field] > 0 else None
            for field in numeric_score_2_fields}

      # Insert into "average" section
      if "average" not in data:
        data["average"] = {}
      data["average"]["numeric score 2.0"] = averages
  ...
  with open("/home/ubuntu/thesis/data/samples/new_samples_no_overlap/test/metadata/agriculture_0_test.json", "r") as file:
  ...
  """directory = "/home/ubuntu/thesis/data/samples/new_samples_no_overlap/generated captions/internvl_8b_text"
  ...
  """for what in ["gt_captions", "metadata", "plots", "time series"]:
    ...
    train_path = f"/home/ubuntu/thesis/data/samples/new_samples_no_overlap/train/{what}"
    test_path = f"/home/ubuntu/thesis/data/samples/new_samples_no_overlap/test/{what}"
  ...
  """directory = "/home/ubuntu/thesis/data/samples/new_samples_no_overlap/train/time series"
