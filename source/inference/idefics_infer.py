...
@lru_cache
def _load_batch_idefics_model(model_name, device):
...
def eval_batch_idefics(prompts, image_files, device, use_image): 
    print(f"use_image={use_image}")
    for i, p in enumerate(prompts):
        if "<image" in p:
            prompts[i] = re.sub(r"<image_(\d+)>", r"<|image_\1|>", p)

    model, processor = _load_batch_idefics_model(FINETUNED_MODEL_PATH, device)
    if use_image:
        content_list = [
...
if __name__ == "__main__":
    run_all_tasks(eval_batch_idefics, DATA_DIR, OUT_DIR) 
